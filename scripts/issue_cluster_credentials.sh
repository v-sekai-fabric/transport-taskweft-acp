...
# and fly CLIs, openssl and python3. Prints subjects and dates, never a key or a token.
set -euo pipefail
...
if [ -z "$ROLE" ]; then
  ROLE=$("$BAO" list -format=json "$MOUNT/roles" 2>/dev/null | tr -d '\r' | "$PYTHON" -c '
import json, subprocess, sys, os
mount, bao = sys.argv[1], sys.argv[2]
roles = json.load(sys.stdin)
for name in sorted(roles, key=lambda n: (0 if "fdb" in n else 1, n)):
    out = subprocess.run([bao, "read", "-format=json", f"{mount}/roles/{name}"], capture_output=True, text=True).stdout
